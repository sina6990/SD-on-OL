...
def prediction_original(model, data):
    ...
    for idx, row in data.iterrows():
        ...
        logging.info(f"Predicted original: {pred_original}, explicit: N/A, implicit: N/A")
...
def prediction_cues(model, data, cities, populations, household_incomes, countries, processed_original, processed_targets):
    targets = [int(v) for v in data['labels'].values]
    preds_explicit = []
    preds_implicit = []
    count = 0
    ...
    for idx, row in data.iterrows():
        ...
        logging.info(f"Predicted original: {pred_original}, explicit: {pred_explicit}, implicit: {pred_implicit}")
