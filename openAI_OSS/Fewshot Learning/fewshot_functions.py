...
class Model:
    ...
    def fewshot_prompting(self, text, positive_example_text, negative_example_text):
        ...
        conversation = []
        system_prompt = """You are given a text preceded by two labeled examples. One example contains hateful content (positive case) and the other does not (negative case). Determine whether the given target text contains elements of hate speech. Just answer with 'Yes' or 'No', and don't add anything else.\n"""
        conversation.append({"role": "system", "content": system_prompt})

        # Including positive and negative examples in the conversation
        conversation.append({"role": "user", "content": f"Example 1\n'{positive_example_text}'\nDoes this text contain elements of hate speech?\n"})
        conversation.append({"role": "assistant", "content": "Yes"})
        conversation.append({"role": "user", "content": f"Example 2\n'{negative_example_text}'\nDoes this text contain elements of hate speech?\n"})
        conversation.append({"role": "assistant", "content": "No"})
        
        user_content = f"Target text:\n{text}"
        conversation.append({"role": "user", "content": user_content})

        full_prompt = f"{system_prompt.strip()}\n\n"
        full_prompt += f"Example 1\n'{positive_example_text}'\nDoes this text contain elements of hate speech?\n"
        full_prompt += f"Yes \n\n"
        full_prompt += f"Example 2\n'{negative_example_text}'\nDoes this text contain elements of hate speech?\n"
        full_prompt += f"No \n\n"
    ...
    def fewshot_explicit_prompting(self, text, cities, countries, index, positive_example_text, negative_example_text, example_cities, example_countries):
        if not hasattr(self.tokenizer, 'chat_template') or self.tokenizer.chat_template is None:
            self.tokenizer.chat_template = (
                "{% if messages[0]['role'] == 'system' %}{% set loop_messages = messages[1:] %}{% set system_message = messages[0]['content'] %}{% else %}{% set loop_messages = messages %}{% set system_message = '' %}{% endif %}"
                "{% for message in loop_messages %}"
        ...
        example_city = example_cities[index % len(example_cities)]
        example_country = example_countries[index % len(example_countries)]
        prefix_example = f"The person living in {example_city} in {example_country} said: "
        user_content_pos = "Example 1\n" + prefix_example + positive_example_text + "\nDoes this text contain elements of hate speech?\n"
        conversation.append({"role": "user", "content": user_content_pos})
        conversation.append({"role": "assistant", "content": "Yes"})
        user_content_neg = "Example 2\n" + prefix_example + negative_example_text + "\nDoes this text contain elements of hate speech?\n"
        conversation.append({"role": "user", "content": user_content_neg})
        conversation.append({"role": "assistant", "content": "No"})

        city = cities[index % len(cities)]
        country = countries[index % len(countries)]
        prefix = f"The person living in {city} in {country} said: "
        user_content = "Target text\n" + prefix + text
        conversation.append({"role": "user", "content": user_content})

        full_prompt = f"{system_prompt.strip()}\n\n"
        full_prompt += user_content_pos
        full_prompt += f"Yes \n\n"
        full_prompt += user_content_neg
        full_prompt += f"No \n\n"
        full_prompt += f"{user_content}\n"
    ...
    def fewshot_implicit_prompting(self, text, populations, household_incomes, index, positive_example_text, negative_example_text, example_populations, example_household_incomes):
        if not hasattr(self.tokenizer, 'chat_template') or self.tokenizer.chat_template is None:
            self.tokenizer.chat_template = (
                "{% if messages[0]['role'] == 'system' %}{% set loop_messages = messages[1:] %}{% set system_message = messages[0]['content'] %}{% else %}{% set loop_messages = messages %}{% set system_message = '' %}{% endif %}"
                "{% for message in loop_messages %}"
        ...
        example_population = example_populations[index % len(example_populations)]
        example_household_income = example_household_incomes[index % len(example_household_incomes)]
        income_parts = example_household_income.split()
        income_value = income_parts[0]
        currency = ' '.join(income_parts[1:]) if len(income_parts) > 1 else ''
        prefix_example = f"The person living in the metropolitan statistical area that has {example_population} people and a median household income of {income_value} {currency} said: "
        user_content_pos = "Example 1\n" + prefix_example + positive_example_text + "\nDoes this text contain elements of hate speech?\n"
        conversation.append({"role": "user", "content": user_content_pos})
        conversation.append({"role": "assistant", "content": "Yes"})
        user_content_neg = "Example 2\n" + prefix_example + negative_example_text + "\nDoes this text contain elements of hate speech?\n"
...
def prediction_original(model, data, positive_example_text, negative_example_text):
    ...
    for idx, row in data.iterrows():
        ...
        pred_original = model.fewshot_prompting(text, positive_example_text, negative_example_text)
        if pred_original == -1:
            undecided += 1
        else:
...
def prediction_cues(model, data, cities, populations, household_incomes, countries, processed_original, positive_example_text, negative_example_text, example_cities, example_populations, example_household_incomes):
    ...
    for idx, row in data.iterrows():
        ...
        text = row['text']
        ground_truth = int(row['labels'])
        pred_explicit = model.fewshot_explicit_prompting(text, cities, countries, idx, positive_example_text, negative_example_text, example_cities, example_countries)
        pred_implicit = model.fewshot_implicit_prompting(text, populations, household_incomes, idx, positive_example_text, negative_example_text, example_populations, example_household_incomes)
